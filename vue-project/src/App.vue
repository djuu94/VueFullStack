  ...
  methods: {
    async performSearch(this: ComponentCustomProperties, searchQuery: string) {
      try {
        const options = {
          method: 'GET',
          url: `https://moviesdatabase.p.rapidapi.com/titles/search/keyword/${searchQuery}`,
          headers: {
  ...
  getFlattenedResults(this: ComponentData & ComponentMethods & ComponentComputed): Movie[] {
    if (Array.isArray(this.searchResults?.results)) {
      return this.searchResults.results.map(result => {
        console.log(result.titleType?.text);
        return {
          idSearch: result.id,
          id: result.id,
          _id: result._id,
          ...
          type: result.titleType?.text || 'Type not available', 
        };
      }) as Movie[];
    } else {
